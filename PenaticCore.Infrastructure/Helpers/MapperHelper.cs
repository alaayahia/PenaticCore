            ...
            foreach (var prop in destProps)
            {
                ...
                foreach (var sourceProp in sourceProps)
                {
                        ...
                        && sourcePropValue != default && !propsToExclude.Contains(propName))
                    {
                        ...
                        if (prop.PropertyType.IsPrimitive || Nullable.GetUnderlyingType(prop.PropertyType) != null
                            || prop.PropertyType == typeof(Decimal)
                            || prop.PropertyType == typeof(String) || prop.PropertyType == typeof(DateTime) || prop.PropertyType.IsEnum)
                        {
                        ...
                        else
                        {
                            ...
                            if (mapMethodInfo != null)
                            {
                                propValue = mapMethodInfo.MakeGenericMethod(sourceProp.PropertyType, prop.PropertyType)
